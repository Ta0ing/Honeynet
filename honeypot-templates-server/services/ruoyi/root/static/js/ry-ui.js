...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 图片预览
            imageViewBase64: function (value, height, width, target) {
                if ($.common.isEmpty(width)) {
                    width = '500';
                }
                if ($.common.isEmpty(height)) {
                ...
                var _target = $.common.isEmpty(target) ? 'self' : target;
                if ($.common.isNotEmpty(value)) {
                    return $.common.sprintf("<img class='img-circle img-xs' data-height='%s' data-width='%s' data-target='%s' src='data:image/jpg;base64,%s'/>", height, width, _target, value);
                } else {
                    return $.common.nullToStr(value);
                }
            },
            // 机动车图片预览
            jdcimageViewBase64: function (value, height, width, target) {
                if ($.common.isEmpty(width)) {
                    width = '720';
                }
                if ($.common.isEmpty(height)) {
            ...
            // 图片预览
            jdcimageView: function (value, height, width, target) {
                if ($.common.isEmpty(width)) {
                    width = '720';
                }
                if ($.common.isEmpty(height)) {
        ...
        modal: {
            ...
            // 弹出层指定宽度
            openClxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            editTcwGx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openFwxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openXqxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openLdxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openDyxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openRyxx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            openRfgx: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度
            open1: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            // 保存信息 刷新表格
            saveNOrefresh: function (url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
                    ...
                    success: function (result) {
                        ...
                        if (result.code == web_status.SUCCESS) {
                            $.modal.close();
                            parent.$.modal.msgSuccess("保存成功,正在刷新数据请稍后……", modal_status.SUCCESS);
                            $.modal.closeLoading();
                        } else {
                            parent.$.modal.msgError(result.msg);
                            $.modal.closeLoading();
                        }
                    }
                };
...
function hideSFzhm(sfzh) {
    if (sfzh == null) {
        return sfzh;
    }
    if (sfzh.length == 18) {
        return sfzh.replace(/(.{4}).*(.{4})/, "$1********$2")
    }
    return sfzh;
}